...
def get_knowledge_docs(page: int = 1, page_size: int = 20, db: Session = Depends(get_db)):
    """5.2 获取知识文档列表接口"""
    skip = (page - 1) * page_size
    total = db.query(KnowledgeDoc).count()
    docs = db.query(KnowledgeDoc).order_by(KnowledgeDoc.created_at.desc()).offset(skip).limit(page_size).all()
...
async def trigger_process_doc(
    ...
    if doc.status == "processing":
        return {"doc_id": doc.id, "status": "processing", "message": "该文档正在处理中，请勿重复点击"}
...
def delete_knowledge_doc(doc_id: int, db: Session = Depends(get_db)):
    ...
    if doc.status == "ready":
        try:
            ...
            print(f"向量库中对应 doc_id={doc_id} 的记忆已被删除")
        except Exception as e:
            print(f"向量库记忆清理失败: {e}")
