...
async def human(request):
    """文本输入（echo 模式），支持 voice/emotion 参数"""
    try:
        ...
        if params['type'] == 'echo':
            ...
            return json_error("unsupported type, only 'echo' is supported")
...
async def admin_create_session(request):
    """外部 API：创建数字人会话"""
    try:
        params = await request.json()
        sessionid = params.get('sessionid')
        ...
        logger.exception('admin_create_session exception:')
        return json_error(str(e))
...
async def admin_delete_session(request):
    ...
    try:
        ...
        logger.exception('admin_delete_session exception:')
        return json_error(str(e))
...
def setup_routes(app):
    ...
    app.router.add_post("/api/admin/session", admin_create_session)
    app.router.add_delete("/api/admin/session/{sessionid}", admin_delete_session)

    # 注册 avatar 生成相关的路由
    setup_avatar_routes(app)

    # web/ 目录已移除（项目使用 Qt6 前端，不需要 LiveTalking 自带前端）
